...
```{r}
unvotes |> 
  count(country, vote) |> # count votes by country
  group_by(country) |> # grouping countries by RCID
  mutate(total = sum(n)) |> # create a column of the total count of votes
  mutate(prop_vote = n/total) |> # create a column of the proportion of votes
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> # include these countries
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + # create the plot with country on x axis and proportion on y
  geom_col(position = position_stack()) + # position the data as a bar chart
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + # create labels for axis and title
  theme_minimal() +  # change the theme of the plot
  scale_fill_viridis_d(end = 0.8) + # sets the color scheme
  coord_flip() # rotate the orientation
```

> Demonstrates the proportion of the G7 countries who voted yes, no, or abstain. For example, France had the highest proportion of "abstain" votes and Germany had the highest proportion of "yes" votes. 
...
> It pulls out the column short which is a short description of what the UN is voting on. 
...
> The missing piece of information is what specifically is being voted on. More specifically whether the votes were on an amendment or not 
...
> This won't be helpful because each short has its own independent value and not grouped by an overall issue. 
...
> There is an uneven number of rows in the two datasets, so the one with the higher number of rows will be reduced to match the dataset with the lower number of rows.
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") # join issues to roll_calls by rcid
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") # join roll_calls to issues by rcid
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") # fully joins both tables to each other
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") # keeps any values that match from both tables
```
...
> Use inner joing because it will maintain only rows with a match from both datasets.
...
```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") |>  #join roll_calls and issues so that just the votes related to the issues data are retained.
...
> They all sort of follow the same trend where when arms control spikes the other two will eventually spike as well. We wonder what what caused the drop after 1990. 
